    ...
    <div className="profile-2-conatiner">
      ...
      <div className="content">
          ...
          <div className="contact-section">
            {
              userInfo.links.map((data) => (
                <a href={`${data?.name === "phone" ? "tel:" : data?.name === "email" ? "mailto:" : data?.name === "telephone" ? "tel+": ""}${data?.link}`} target="_blank"  className={(data.name === "phone" || data.name === "telephone") ? `w-50 single-contact` : `w-100 single-contact`}>
        ...
        <div className="contact-inner">
          ...
          <button style={{background: `${buttonInfo.colors.bg}`, color: `${buttonInfo.colors.color}`, boxShadow: `0 0 5px ${buttonInfo.colors.shadow}`}} className="add-contact-button">{buttonInfo?.info?.text || "ADD TO CONTACT"}</button>
        </div>
      </div>
    </div>
  );
