...
const PendingProfile = () => {
    ...
    const fetchData = async (currentPage, itemPerPage) => {
        try {
            const data = await SecureFetch.get(`${apiBaseURL}/api/profile/get-profile-by-user-filter?isApproved=false&pageNo=${currentPage}&limit=${itemPerPage}`);
            ...
            <div className="card-requested-table-section">
                <div className="table-conatiner">
                    {allProfileData.length > 0 ? (
                        ...
                        <div className="alert-primary p-5 text-center">
                            <div className="display-1">🤨</div> No Pending User
                        </div>
                    )}
                </div>
