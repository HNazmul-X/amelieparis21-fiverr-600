...
function LeftSide({ setTemplateDataReloader, templateDataReloader }) {
  ...
  // useEffects
  useEffect(async () => {
    const x = await SecureFetch.get(
      `${apiBaseURL}/api/user/get-user-by-profileId/${profileId}?select=_id`
    );
    setTemplatedUser(x);
  ...
  const handleProfileCreation = async (type) => {
    error.profileTemplate = profileContext.profileTemplateId
      ? null
      : "Please Select a Profile template";
    error.profilePics =
      profileContext?.userPics?.profile?.dataURL ||
      profileContext?.userPics?.cover?.dataURL
        ? null
        : "please upload profile pic and cover photo";
    error.buttonInfo =
      profileContext?.buttonInfo?.info?.text ||
      profileContext?.buttonInfo?.info?.link
        ? null
        : "please Fill up Button information";
    setError({ ...error });
    ...
    if (Object.values(error).filter((item) => item).length > 0) {
      ...
      try {
        ...
        imageFormData.append(
          "profilePic",
          profileContext?.userPics?.profile?.file
        );
        ...
        const templateDataGathering = {
          ...
          photos: {
            ...
            profile:
              profileContext?.userPics.profile.dataURL.split(apiBaseURL)[1],
          },
        };

        // if template action type is create
        if (type === "create") {
          const uploadedPics = await SecureFetch.post(
            `${apiBaseURL}/api/profile-template/upload-template-images`,
            imageFormData
          );
        ...
        if (type === "create" && !uploadedUserPic.error) {
          const r_data = await SecureFetch.post(
            `${apiBaseURL}/api/profile-template/create-profile-template/${templatedUser?.user?._id}`,
            templateDataGathering
          );
          checkingDataOrError(r_data, "Creation");
          setTemplateDataReloader(Math.random());
        } else if (type === "update") {
          const r_data = await SecureFetch.post(
            `${apiBaseURL}/api/profile-template/updated-profileTemplate/${templatedUser?.user?.profileTemplate}`,
            templateDataGathering
          );
  ...
  const handleDeletingProfile = (id, userId) => {
    swal({
      ...
      if (value) {
        ...
        const r_data = await SecureFetch.post(
          `${apiBaseURL}/api/profile-template/delete-profile-template`,
    ...
    <>
      <div className="left-side-container db-template">
        <SelectProfileTemplate
          ...
          open={open}
          title={"Sélectionnez votre design : "}
        />
        <SetupProfilePics
          templatedUser={templatedUser}
          handelOpen={handelOpen}
          open={open}
          title={"Configurez  vos images de profil :"}
        />
        <SetUpPersonalInformation
          handelOpen={handelOpen}
          open={open}
          title={"Configurez les informations personnelles :"}
        />
        <SetupSocialLink
          setIconPopupShow={setIconPopupShow}
          handelOpen={handelOpen}
          open={open}
          title={"Configurez les liens : "}
        />
        <SetupProfileButton
          setIconPopupShow={setIconPopupShow}
          handelOpen={handelOpen}
          open={open}
          title={"Configurer le bouton  Ajouter aux contacts "}
        />
        <div className="text center d-flex justify-content-evenly">
          {!templatedUser?.user?.profileTemplate ? (
            <>
              ...
              <button
                onClick={() => handleProfileCreation("create")}
                className="create-profile btn btn-primary rounded-pill btn-lg px-4"
              >
            ...
            <>
              {auth?.user?.isAdmin && (
                <button
                  onClick={() =>
                    handleDeletingProfile(
                      templatedUser?.user?.profileTemplate,
                      templatedUser?.user?._id
                    )
                  }
              ...
              <button
                onClick={handleProfileCreation.bind(this, "update")}
                className="create-profile btn btn-primary rounded-pill btn-lg px-4"
              >
                {spinners.update && (
                  <div
              ...
              <button
                className="btn-success btn rounded-pill px-4 fs-5"
                onClick={() => navigate("/u/" + templatedUser?.user?.username)}
              >
                Preview
              </button>
            </>
      ...
      {iconPopupShow ? (
        <SocialLinkSelection
          ...
          onClose={() => setIconPopupShow(!iconPopupShow)}
        />
      ) : null}
    </>
  );
